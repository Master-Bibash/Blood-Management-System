...
function isEligibleToDonateBlood($name, $contact, $email, $conn) {
    // Check if there is any record in the 'donor' table for the same donor in the last 30 days
    $check_sql = "SELECT collection FROM donor WHERE username = ? AND contact = ? AND email = ? AND collection >= NOW() - INTERVAL 30 DAY";
    ...
    if (mysqli_num_rows($result) > 0) {
        // There is a record in the 'donor' table for the same donor in the last 30 days
...
if (isset($_POST['submit'])) {
    ...
    if ($collection != $row['collection'] && strtotime($row['collection']) > strtotime('-30 days')) {
...
<button type="submit" class="btn btn-success" name="submit" id="submitButton">Submit</button>
    ...
    <script>
    // Client-side validation for collection date
    var collectionDateInput = document.getElementById('collectionDate');
    var dateErrorMessage = document.getElementById('dateErrorMessage');

    collectionDateInput.addEventListener('change', function () {
        var selectedDate = new Date(collectionDateInput.value);
        var currentDate = new Date();

        if (selectedDate < currentDate) {
            dateErrorMessage.textContent = "Collection date cannot be in the past.";
            collectionDateInput.value = ""; // Clear the input field
